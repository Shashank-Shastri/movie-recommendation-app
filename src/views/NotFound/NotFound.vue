    ...
    <div id="error">
        ...
        <div class="error">
            ...
            <div class="error__description">
                It looks like you are lost in exploration
            </div>
            <router-link :to="{ name: 'Home' }"
                ><button class="error__button">Home</button></router-link
            >
        </div>
...
export default {
    ...
    beforeMount() {
        for (let i = 1; i <= 3; i++)
            this.moonCraters.push(`moon__crater moon__crater${i}`);
        for (let i = 1; i <= 100; i++) {
            this.randomStars.push(
                `animation: twinkle ${Math.random() * 5 +
                    5}s linear ${Math.random() * 5 +
                    5}s infinite; top: ${Math.random() *
                    screen.height}px; left: ${Math.random() * screen.width}px;`
            );
        }
    },
    mounted() {
